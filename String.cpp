...
char* trim(char* desc,char* src,char* seps)
{
  ...
  char* nToken = NULL;
  token = strtok_s(src, seps,&nToken);
  while( token != NULL )
  {
	/* While there are tokens in "string" */
	strcat_s(desc,sizeof(*desc),token);
	char* nToken = NULL;
...
unsigned int ip_int_cvt(char* ipadd){
  unsigned char buf[sizeof(struct in_addr)];
  return inet_pton(AF_INET,ipadd,buf);
}
...
void parseHexIPtoStr(char* strip,char const* hexIP){
  ...
  for(int j=0;j<largeUnitCount;j++){
   ...
   sprintf_s(strip,sizeof(*strip),"%d.%d.%d.%d",intsaver[0],intsaver[1],intsaver[2],intsaver[3]);
}
