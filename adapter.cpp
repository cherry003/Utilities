...
int getDomainInfo(domain* p_dm){
   ...
   if(ERROR_SUCCESS==GetNetworkParams((FIXED_INFO*)pbBuf1, &ulLen1))
   {
	 ...
	 strcpy_s(p_dm->domain_name,sizeof(*(p_dm->domain_name)),FixedInfo->DomainName);
	 strcpy_s(p_dm->host_name, sizeof(*(p_dm->host_name)), FixedInfo->HostName);
...
int getAdapterInfo(adapter_container &adaq){
   ...
   if(ERROR_SUCCESS==GetAdaptersInfo((IP_ADAPTER_INFO*)pbBuf1, &ulLen1))
   {
	  ...
	  while(pAdapter){
		 ...
		 strcpy_s(name,sizeof(*name),pAdapter->AdapterName);
...
int getBcastAddPerAdapter(num_container &bcastq){
   ...
   if(ERROR_SUCCESS==GetAdaptersInfo((IP_ADAPTER_INFO*)pbBuf1, &ulLen1))
   {
	  ...
	  while(pAdapter){
	   ...
	   while(ipstr!=NULL){
		 strcpy_s(ipaddrcopy,sizeof(*ipaddrcopy),ipstr->IpAddress.String);
		 strcpy_s(maskcopy,sizeof(*maskcopy),pAdapter->IpAddressList.IpMask.String);
		 ...
		 else
		 {
			...
			u32_t IPaddr= inet_pton(AF_INET,ipaddrcopy,buf);
			u32_t mask= inet_pton(AF_INET,maskcopy,buf);
   ...
   if(ERROR_SUCCESS==GetAdaptersInfo((IP_ADAPTER_INFO*)pbBuf1, &ulLen1))
   {
	  ...
	  while(pAdapter){
		...
		while(ipstr!=NULL){
		 ...
		 {
           unsigned char buf[sizeof(struct in_addr)];
           u32_t uipadd= inet_pton(AF_INET,ipaddrcopy,buf);
